...
class ManageQuestions extends Page implements HasTable
{
    ...
    protected static string $view = 'filament.lecturer.pages.manage-questions';
    ...
    public function table(Table $table): Table
    {
        return $table
            ...
            ->columns([
                Tables\Columns\TextColumn::make('sort_order')->label(__('lecturer.col_sort_order'))->sortable()->width(50),
                ...
                Tables\Columns\TextColumn::make('content')->limit(80)->searchable()->label(__('lecturer.qbank_field_content_short')),
                Tables\Columns\TextColumn::make('marks')->sortable(),
                Tables\Columns\TextColumn::make('negative_marks')->label(__('lecturer.col_neg_marks')),
            ])
            ->actions([
                ...
                Tables\Actions\Action::make('save_to_bank')
                    ->label(__('lecturer.action_save_to_bank'))
                    ->icon('heroicon-o-archive-box-arrow-down')
                    ->color('gray')
                    ->form([
                        Forms\Components\Select::make('collection_id')
                            ->label(__('lecturer.qbank_collection_optional'))
                    ...
                    ->action(function (array $data, Question $record): void {
                        ...
                        if ($saved) {
                            Notification::make()->title(__('lecturer.qbank_notif_saved_to_bank'))->success()->send();
                        } else {
                            Notification::make()->title(__('lecturer.qbank_notif_already_in_bank'))->warning()->send();
                        }
                    }),
            ...
            ->emptyStateHeading(__('lecturer.manage_q_empty_heading'))
            ->emptyStateDescription(__('lecturer.manage_q_empty_desc'))
    ...
    public function getHeaderActions(): array
    {
        return [
            Actions\Action::make('back')
                ->label(__('lecturer.btn_back_to_quiz'))
                ->url(fn() => QuizResource::getUrl('edit', ['record' => $this->quizId]))
                ->color('gray'),
            Actions\Action::make('ai_generate')
                ->label(__('lecturer.generate_with_ai'))
                ->icon('heroicon-o-sparkles')
                ->color('warning')
                ->url(fn() => route('filament.lecturer.pages.ai-generator', ['quiz_id' => $this->quizId])),
            Actions\Action::make('add_question')
                ->label(__('lecturer.add_question'))
    ...
    private function questionForm(?Question $record = null): array
    {
        return [
            ...
            Forms\Components\Textarea::make('content')
                ->label(__('lecturer.qbank_field_content_short'))->required()->rows(3)->columnSpanFull(),
            ...
            Forms\Components\Repeater::make('options')
                ->label(__('lecturer.qbank_section_options'))
                ->schema([
            ...
            Forms\Components\Repeater::make('blank_answers')
                ->label(__('lecturer.qbank_section_answers'))
                ->schema([
                    Forms\Components\TextInput::make('answer')->required(),
                ])
                ->visible(fn (Forms\Get $get) => in_array($get('type'), ['fill_blank', 'short_answer']))
