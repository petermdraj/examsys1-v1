...
<div class="wrap cmp-wrap-pt8">
  ...
  @if($featured->isEmpty())
    <div class="cmp-empty">
      <p>{{ __('common.no_quizzes_competition') }}</p>
    </div>
  @else
  ...
  {{-- CREATOR CTA STRIP --}}
  @php
      $ctaUrl = $platformSettings->lecturer_cta_url ?: url('/lecturer');
  @endphp
  <div class="creator-cta-dark">
    <div class="creator-cta-content">
      ...
      <div class="creator-cta-text">
        <div class="creator-cta-title">{{ $platformSettings->lecturer_cta_title }}</div>
        <div class="creator-cta-sub">{{ $platformSettings->lecturer_cta_sub }}</div>
      </div>
      <a href="{{ $ctaUrl }}" class="btn btn-accent cmp-cta-btn">{{ $platformSettings->lecturer_cta_btn }}</a>
    </div>
