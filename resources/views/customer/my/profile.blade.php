...
<div class="student-layout">
  ...
  <div class="card myp-card">
    ...
    <form method="POST" action="{{ route('profile.notifications') }}">
      ...
      <div class="myp-notif-col">
        ...
        <label class="toggle-row">
          <div>
            <div class="myp-notif-label-name">{{ __('common.notify_purchases') }}</div>
            <div class="muted myp-notif-label-sub">{{ __('common.notify_purchases_sub') }}</div>
          </div>
          <input type="checkbox" name="notify_purchases" value="1"
                 {{ ($prefs['notify_purchases'] ?? true) ? 'checked' : '' }}
                 class="toggle-input">
        </label>
  ...
  @if($orders->count() > 0)
  <div class="card myp-card">
    <h2 class="myp-section-h2-purchase">{{ __('common.purchase_history_heading') }}</h2>
    @foreach($orders as $order)
      <div class="myp-order-row">
        <div class="myp-order-info">
          <div class="myp-order-title">{{ $order->quiz?->title ?? 'Quiz #'.$order->id }}</div>
          <div class="muted myp-order-date">{{ $order->paid_at?->format('d M Y') ?? $order->created_at->format('d M Y') }}</div>
        </div>
        <div class="myp-order-amount">{{ $platformSettings->currency_symbol }}{{ number_format($order->amount, 2) }}</div>
        <span class="badge badge-free myp-order-badge">{{ __('common.paid_badge') }}</span>
      </div>
    @endforeach
  </div>
  @endif
