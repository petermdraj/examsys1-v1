...
<div class="wrap">
  <div class="detail-grid">
    ...
    <div>
      ...
      <div class="card creator-card">
        ...
        <div class="creator-meta">
          ...
          <div class="sec qs-creator-sec">{{ __('quiz.creator_quizzes_published', ['count' => $quiz->creator->quizzes()->published()->count()]) }}</div>
        </div>
      </div>
      ...
      <div class="card sample">
        ...
        <div class="sample-lock-cta">
          ...
          @auth
            ...
            @else
              <a href="{{ route('quiz.checkout', $quiz->slug) }}" class="btn btn-primary">{{ __('quiz.buy_for_price', ['symbol' => $platformSettings->currency_symbol, 'price' => number_format($quiz->price, 0)]) }}</a>
            @endif
          @else
            <a href="{{ route('register') }}" class="btn btn-primary">{{ __('quiz.signup_to_enroll') }}</a>
          @endauth
        </div>
        @endif
      </div>
    ...
    <aside>
      ...
      <div class="card buy">
        @if($quiz->isFree())
          <div class="buy-price">{{ __('quiz.card_free') }}</div>
        @else
        ...
        @elseif($quiz->isFree())
          @auth
            <form method="POST" action="{{ route('attempt.start', $quiz->slug) }}" class="qs-mt20">@csrf
              <button type="submit" class="btn btn-primary btn-lg btn-block">{{ __('quiz.enroll_start_exam') }}</button>
            </form>
          @else
            <a href="{{ route('register') }}" class="btn btn-primary btn-lg btn-block qs-mt20">{{ __('quiz.signup_to_enroll') }}</a>
          @endauth
        @else
          @auth
            <a href="{{ route('quiz.checkout', $quiz->slug) }}" class="btn btn-primary btn-lg btn-block qs-mt20-block">{{ __('quiz.buy_for_price', ['symbol' => $platformSettings->currency_symbol, 'price' => number_format($quiz->price, 0)]) }}</a>
          @else
            <a href="{{ route('register') }}" class="btn btn-primary btn-lg btn-block qs-mt20">{{ __('quiz.signup_to_buy') }}</a>
          @endauth
        @endif
