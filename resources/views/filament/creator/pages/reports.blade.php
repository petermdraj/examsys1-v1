...
<div class="rp-page">
    ...
    <div class="rp-sel">
        <div class="rp-sel-left">
            <div class="rp-sel-label">{{ __('creator.rp_viewing_for') }}</div>
        ...
        <div class="rp-sel-meta">
            ...
            <span class="rp-sel-count">{{ $quiz->price > 0 ? $sym.$quiz->price : __('creator.rp_free') }}</span>
        </div>
        @endif
    </div>
    ...
    <div class="rp-grid-4">
        ...
        <div class="rp-card rp-kpi">
            ...
            <div class="kpi-hdr">
                <div class="kpi-label">{{ __('creator.rp_total_attempts') }}</div>
            ...
            <div class="kpi-sub">
                <span class="kpi-sub-passed">{{ __('creator.rp_n_passed', ['n' => $stats['passed']]) }}</span>
                &nbsp;·&nbsp;
                <span class="kpi-sub-failed">{{ __('creator.rp_n_failed', ['n' => $stats['failed']]) }}</span>
            </div>
        ...
        <div class="rp-card rp-kpi">
            ...
            <div class="kpi-hdr">
                <div class="kpi-label">{{ __('creator.rp_avg_score') }}</div>
            ...
            <div class="kpi-sub">{{ __('creator.rp_top') }} <strong class="kpi-sub-dim">{{ $stats['top_score'] }}%</strong> &nbsp;·&nbsp; {{ __('creator.rp_low') }} <strong class="kpi-sub-dim">{{ $stats['lowest_score'] }}%</strong></div>
        ...
        <div class="rp-card rp-kpi">
            ...
            <div class="kpi-hdr">
                <div class="kpi-label">{{ __('creator.rp_pass_rate') }}</div>
        ...
        <div class="rp-card rp-kpi">
            ...
            <div class="kpi-hdr">
                <div class="kpi-label">{{ __('creator.rp_avg_time') }}</div>
            ...
            <div class="kpi-sub">{{ $quiz->duration_minutes ? __('creator.rp_of_min_limit', ['n' => $quiz->duration_minutes]) : __('creator.rp_no_time_limit') }}</div>
    ...
    <div class="rp-grid-dist">
        ...
        <div class="rp-section">
            <div class="rp-section-title">{{ __('creator.rp_score_distribution') }}</div>
            ...
            @endforeach
            @else
            <div class="rp-nodata">{{ __('creator.rp_no_data_yet') }}</div>
            @endif
        </div>
        ...
        <div class="rp-section rp-section-tbl">
            <div class="rp-tbl-hdr">
                <div class="rp-section-title rp-section-title-inline">{{ __('creator.rp_recent_attempts') }}</div>
            ...
            @else
            <div class="rp-noattempts">{{ __('creator.rp_no_attempts_yet') }}</div>
            @endif
        </div>
    </div>
    ...
    {{-- ── Quiz config ── --}}
    <div class="rp-section">
        <div class="rp-section-title">{{ __('creator.rp_quiz_config') }}</div>
        <div class="rp-grid-meta">
            @php
                $meta = [
                    ['label'=>__('creator.rp_meta_questions'),    'value'=>$quiz->total_questions,          'color'=>'#8b5cf6', 'icon'=>'M8.228 9c.549-1.165 2.03-2 3.772-2 2.21 0 4 1.343 4 3 0 1.4-1.278 2.575-3.006 2.907-.542.104-.994.54-.994 1.093m0 3h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z'],
                    ['label'=>__('creator.rp_meta_total_marks'),  'value'=>$quiz->total_marks,              'color'=>'#f59e0b', 'icon'=>'M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z'],
                    ['label'=>__('creator.rp_meta_pass_pct'),     'value'=>$quiz->pass_percentage.'%',      'color'=>'#10b981', 'icon'=>'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z'],
                    ['label'=>__('creator.rp_meta_duration'),     'value'=>$quiz->duration_minutes ? $quiz->duration_minutes.' min' : __('creator.rp_unlimited'),'color'=>'#3b82f6','icon'=>'M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z'],
                    ['label'=>__('creator.rp_meta_max_attempts'), 'value'=>$quiz->max_attempts ?? __('creator.rp_unlimited'),'color'=>'#6366f1','icon'=>'M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15'],
                    ['label'=>__('creator.rp_meta_price'),        'value'=>$quiz->price > 0 ? $sym.$quiz->price : __('creator.rp_free'),'color'=>'#ec4899','icon'=>'M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z'],
                ];
            @endphp
            @foreach($meta as $m)
    ...
    <div class="rp-empty">
        ...
        <a href="{{ route('filament.creator.resources.quizzes.create') }}" class="rp-empty-cta">
            + Create Quiz
        </a>
    </div>
    @else
