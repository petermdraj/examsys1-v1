@php
    ...
    $creditsFree = auth()->user()->ai_credits_free_remaining ?? 0;
    // Bank collections for filter dropdown
    $bankCollections = \App\Models\QuestionCollection::where('creator_id', auth()->id())
...
{{-- ══ STATS ══════════════════════════════════════════════════════ --}}
<div style="display:grid;grid-template-columns:repeat(3,1fr) auto;background:#E5E7EB;border-radius:14px;overflow:hidden;border:1px solid #E5E7EB;gap:1px;">
    <div class="qqm-stat-cell">
        <p class="qqm-stat-label">{{ __('creator.stat_questions') }}</p>
        <p class="qqm-stat-value" style="color:#6366F1;" x-text="questions.length">{{ $existingQs->count() }}</p>
    </div>
    <div class="qqm-stat-cell">
        <p class="qqm-stat-label">{{ __('creator.stat_total_marks') }}</p>
        <p class="qqm-stat-value" style="color:#8B5CF6;" x-text="totalMarks">{{ $existingQs->sum('marks') }}</p>
    </div>
    <div class="qqm-stat-cell">
        <p class="qqm-stat-label">{{ __('creator.stat_ai_credits') }}</p>
        <p class="qqm-stat-value" style="color:#F59E0B;" x-text="creditsFree">{{ $creditsFree }}</p>
    </div>
...
<div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;align-items:start;">
    ...
    <div style="border-radius:14px;border:1.5px solid #DDD6FE;box-shadow:0 1px 6px rgba(109,40,217,.07);overflow:visible;">
        <button type="button" @click="aiOpen=!aiOpen"
            ...
            <div style="flex:1;">
                <p style="font-size:13px;font-weight:700;color:#fff;margin:0;">{{ __('creator.generate_with_ai') }}</p>
                <p style="font-size:11px;color:rgba(255,255,255,.7);margin:2px 0 0;">{{ __('creator.ai_auto_create_desc') }}</p>
            </div>
        ...
        <div x-show="aiOpen" style="padding:18px 18px 20px;background:#FAFAFA;display:flex;flex-direction:column;gap:13px;border-radius:0 0 13px 13px;">
            <div>
                ...
                <textarea x-model="aiPrompt" rows="2" placeholder="{{ __('creator.ai_prompt_placeholder') }}"
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
                <div>
                    <label class="qqm-field-label-sm">{{ __('creator.ai_count_label') }}</label>
                    <div style="position:relative;">
                        <select x-model.number="aiCount" style="width:100%;border:1.5px solid #E5E7EB;border-radius:8px;padding:8px 28px 8px 11px;font-size:13px;background:#fff;color:#111827;appearance:none;-webkit-appearance:none;-moz-appearance:none;outline:none;cursor:pointer;box-sizing:border-box;">
                            @foreach([5,10,15,20,25,30] as $n)<option value="{{ $n }}">{{ $n }}</option>@endforeach
                        </select>
                ...
                <div>
                    <label class="qqm-field-label-sm">{{ __('creator.ai_type_label') }}</label>
                    <div style="position:relative;">
                        <select x-model="aiType" style="width:100%;border:1.5px solid #E5E7EB;border-radius:8px;padding:8px 28px 8px 11px;font-size:13px;background:#fff;color:#111827;appearance:none;-webkit-appearance:none;-moz-appearance:none;outline:none;cursor:pointer;box-sizing:border-box;">
                            <option value="mixed">{{ __('creator.ai_type_mixed') }}</option>
                            <option value="mcq_single">{{ __('creator.ai_type_mcq_single') }}</option>
                            <option value="mcq_multiple">{{ __('creator.ai_type_mcq_multiple') }}</option>
                            <option value="true_false">{{ __('creator.ai_type_true_false') }}</option>
                            <option value="fill_blank">{{ __('creator.ai_type_fill_blank') }}</option>
                        </select>
                        <svg style="position:absolute;right:9px;top:50%;transform:translateY(-50%);pointer-events:none;" width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="#9CA3AF" stroke-width="2.5"><path d="M6 9l6 6 6-6"/></svg>
                    </div>
                </div>
                <div style="grid-column:span 2;">
                    <label class="qqm-field-label-sm">{{ __('creator.aig_language_label') }}</label>
                    <div style="position:relative;">
            ...
            <div x-show="aiDone" style="display:flex;gap:8px;padding:9px 12px;background:#F0FDF4;border:1px solid #BBF7D0;border-radius:8px;">
                ...
                <span style="font-size:12px;color:#15803D;font-weight:600;"><span x-text="aiAdded"></span> {{ __('creator.ai_questions_added') }}</span>
            </div>

            {{-- ★ GENERATE BUTTON --}}
            <div @click="creditsFree > 0 && !aiLoading && generateAI()"
                :style="'display:inline-flex;align-items:center;gap:7px;padding:8px 14px;margin-top:12px;border-radius:8px;cursor:pointer;user-select:none;background:linear-gradient(135deg,#7C3AED,#6366F1);box-shadow:0 2px 8px rgba(109,40,217,.3);' + ((aiLoading||creditsFree<=0)?'opacity:.5;cursor:not-allowed;box-shadow:none;':'')">
                ...
                <span style="font-size:13px;font-weight:700;color:#fff;" x-text="aiLoading ? '{{ __('creator.ai_generating') }}' : '{{ __('creator.ai_generate_btn') }}'"></span>
                <span style="font-size:11px;color:rgba(255,255,255,.65);padding-left:4px;border-left:1px solid rgba(255,255,255,.25);" x-text="creditsFree > 0 ? creditsFree + ' credits' : 'No credits'"></span>
            </div>
        </div>
    </div>
    ...
    <div class="rounded-xl overflow-hidden border border-indigo-200 bg-white" style="box-shadow:0 2px 12px rgba(99,102,241,.1);">
        ...
        <button type="button" @click="toggleBankOpen()"
            ...
            <div class="flex-1 min-w-0">
                <div class="font-bold text-white" style="font-size:13px;">{{ __('creator.import_from_bank') }}</div>
            ...
            <span x-show="bankSelected.length > 0"
                ...
                x-text="bankSelected.length + ' {{ __('creator.bank_selected_suffix') }}'"></span>
        ...
        <div x-show="bankOpen">
            ...
            <div class="flex flex-row items-start gap-2 border-b border-amber-200" style="padding:9px 14px;background:#FFFBEB;">
                ...
                <span class="text-amber-800" style="font-size:12px;">{{ __('creator.complete_steps_first') }}, then return here to import questions into your quiz.</span>
            </div>
            @endif
                    ...
                    <div class="text-center" style="padding:30px 16px;">
                        ...
                        <div class="text-gray-400" style="font-size:12px;">
                            <template x-if="bankAllItems.length===0">
                                <span>Add questions via <a href="/creator/question-banks" class="text-indigo-500 font-semibold">Question Bank</a></span>
                            </template>
                            <template x-if="bankAllItems.length>0">
        ...
        generateAI() {
            ...
            es.onmessage = (e) => {
                ...
                if (d.done) {
                    ...
                    if (this.creditsFree>0) this.creditsFree--;
                    es.close();
