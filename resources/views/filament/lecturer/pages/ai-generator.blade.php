...
'use strict';
const _aigI18n = {
    errSelectQuiz:   @js(__('lecturer.aig_err_select_quiz')),
    errEnterPrompt:  @js(__('lecturer.aig_err_enter_prompt')),
    errConnection:   @js(__('lecturer.aig_err_connection')),
    questionsSaved:  @js(__('lecturer.aig_questions_saved')),
    removedNote:     @js(__('lecturer.aig_removed_note')),
    allSavedNote:    @js(__('lecturer.aig_all_saved_note')),
    collapseAll:     @js(__('lecturer.aig_collapse_all')),
    expandAll:       @js(__('lecturer.aig_expand_all')),
    streamingMore:   @js(__('lecturer.aig_streaming_more')),
    answerLabel:     @js(__('lecturer.aig_answer_label')),
    explanationLabel:@js(__('lecturer.aig_explanation_label')),
    streamingThinking: @js(__('lecturer.aig_streaming_thinking')),
    streamingDoneOf: @js(__('lecturer.aig_streaming_done_of')),
};
const _quizMeta = @js(
    \App\Models\Quiz::where('lecturer_id', auth()->id())
...
<div x-data="aiGen" class="aig-wrap" style="font-family:'Inter',system-ui,sans-serif;">
    ...
    <div style="display:grid;grid-template-columns:370px 1fr;gap:20px;align-items:start;">
        ...
        <div class="aig-card">
            ...
            <div class="aig-flex-center" style="gap:12px;padding:16px 20px;background:linear-gradient(135deg,#faf5ff,#f5f3ff);border-bottom:1.5px solid #ede9fe;">
                ...
                <div>
                    <div style="font-size:14px;font-weight:700;color:#1e1b4b;line-height:1.2;">{{ __('lecturer.aig_settings_heading') }}</div>
                    <div style="font-size:11px;color:#a78bfa;margin-top:2px;">{{ __('lecturer.aig_settings_sub') }}</div>
                </div>
            </div>
            ...
            <div class="aig-col" style="padding:20px;gap:16px;">
                ...
                {{-- Save To tabs --}}
                <div>
                    <span class="aig-label">{{ __('lecturer.aig_save_to') }}</span>
                ...
                <div x-show="saveMode==='bank'" x-transition x-data="{ adding: false, newName: '', saving: false }">
                    <div class="aig-flex-between" style="margin-bottom:7px;">
                        <span class="aig-label" style="margin-bottom:0;">{{ __('lecturer.qbank_collection_optional') }}</span>
                    ...
                    <div x-show="adding" x-transition class="aig-flex-center" style="gap:8px;margin-top:6px;">
                        <input type="text" x-model="newName" placeholder="Collection name…"
                            @keydown.enter.prevent="
                                ...
                                saving = true;
                                fetch('/lecturer/api/bank-collections', {
                                    method: 'POST',
                ...
                {{-- Prompt --}}
                <div>
                    <span class="aig-label">{{ __('lecturer.aig_prompt_label') }}</span>
                    <textarea x-model="prompt" rows="3" class="aig-textarea"
                        placeholder="{{ __('lecturer.aig_prompt_placeholder') }}"></textarea>
                </div>

                {{-- 2×2 settings grid --}}
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
                    <div>
                        <span class="aig-label">{{ __('lecturer.aig_questions_label') }}</span>
                        <input type="number" x-model.number="count" min="1" step="1" class="aig-select" style="width:100%;">
                    </div>
                    <div>
                        <span class="aig-label">{{ __('lecturer.aig_difficulty_label') }}</span>
                        <select x-model="difficulty" class="aig-select">
                            <option value="easy">{{ __('lecturer.qbank_diff_easy') }}</option><option value="medium" selected>{{ __('lecturer.qbank_diff_medium') }}</option>
                            <option value="hard">{{ __('lecturer.qbank_diff_hard') }}</option><option value="mixed">{{ __('lecturer.ai_type_mixed') }}</option>
                        </select>
                    </div>
                    <div>
                        <span class="aig-label">{{ __('lecturer.aig_type_label') }}</span>
                        <select x-model="type" class="aig-select">
                            <option value="mixed">{{ __('lecturer.ai_type_mixed') }}</option><option value="mcq_single">{{ __('lecturer.ai_type_mcq_single') }}</option>
                            <option value="mcq_multiple">{{ __('lecturer.ai_type_mcq_multiple') }}</option><option value="fill_blank">{{ __('lecturer.ai_type_fill_blank') }}</option>
                            <option value="true_false">{{ __('lecturer.ai_type_true_false') }}</option>
                        </select>
                    </div>
                    <div>
                        <span class="aig-label">{{ __('lecturer.aig_marks_label') }}</span>
                ...
                {{-- Language --}}
                <div>
                    <span class="aig-label">{{ __('lecturer.aig_language_label') }}</span>
                ...
                <div x-show="['mcq_single','mcq_multiple','mixed'].includes(type)" x-transition>
                    <span class="aig-label">{{ __('lecturer.aig_options_label') }}</span>
                ...
                <label class="aig-flex-center" style="gap:12px;cursor:pointer;padding:12px 14px;border-radius:11px;background:var(--aig-card-sub);border:1.5px solid var(--aig-border);">
                    ...
                    <div>
                        <div style="font-size:13px;font-weight:600;color:var(--aig-text);line-height:1.2;">{{ __('lecturer.aig_neg_marking_label') }}</div>
                        <div class="aig-subtext">{{ __('lecturer.aig_neg_marking_sub') }}</div>
                    </div>
                </label>
                ...
                <button type="button" @click="generate()" :disabled="loading" class="aig-btn-generate">
                    ...
                    <span x-text="loading ? @js(__('lecturer.aig_generating')) : @js(__('lecturer.aig_generate_btn'))"></span>
                </button>
        ...
        <div class="aig-min0">
            ...
            <div x-show="!loading && !output && !questions.length" x-transition
                ...
                <div style="font-size:18px;font-weight:800;color:#1e1b4b;margin-bottom:6px;letter-spacing:-.01em;">{{ __('lecturer.aig_empty_heading') }}</div>
                <div style="font-size:13px;color:#9ca3af;line-height:2;text-align:center;">
                    {{ __('lecturer.aig_empty_sub') }}&nbsp;<span class="aig-gen-badge">
                ...
                <div class="aig-flex-center" style="gap:10px;width:100%;max-width:340px;margin:22px auto 16px;">
                    ...
                    <span style="font-size:10px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:#c4b5fd;white-space:nowrap;">{{ __('lecturer.aig_sample_divider') }}</span>
                ...
                <div style="padding:24px;">
                    <div class="aig-flex-between" style="margin-bottom:18px;">
                        <div class="aig-flex-center" style="gap:14px;">
                            ...
                            <div>
                                <div style="font-size:15px;font-weight:700;color:#1e1b4b;">{{ __('lecturer.aig_streaming_heading') }}</div>
                        ...
                        <button type="button"
                            @click="
                                ...
                                const dest = saveMode === 'bank'
                                    ? '{{ route('filament.lecturer.resources.question-banks.index') }}'
                                    : '{{ route('filament.lecturer.resources.quizzes.index') }}';
                                $wire.saveAndRedirect(count, colName, dest);
                            "
                            class="aig-btn-save-done"
                            onmouseover="this.style.opacity='.9'" onmouseout="this.style.opacity='1'">
                            {{ __('lecturer.aig_save_done') }}
                        </button>
                    </div>
                </div>
    ...
    <div class="aig-modal-backdrop" x-show="showRemoveModal" x-transition.opacity
        ...
        <div class="aig-modal-box" @click.stop x-transition:enter="aig-modal-in">
            ...
            <div class="aig-modal-title">{{ __('lecturer.aig_modal_title') }}</div>
            <div class="aig-modal-sub">{{ __('lecturer.aig_modal_body') }}</div>
            <div class="aig-modal-preview" x-text="removeTargetContent"></div>
            <div class="aig-modal-actions">
                <button type="button" class="aig-modal-cancel" @click="showRemoveModal = false">{{ __('lecturer.aig_modal_cancel') }}</button>
